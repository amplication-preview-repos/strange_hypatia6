...
export class LeadResolverBase {
  ...
  @common.UseInterceptors(AclFilterResponseInterceptor)
  @graphql.ResolveField(() => [Deadline], { name: "deadlines" })
  @nestAccessControl.UseRoles({
    resource: "Deadline",
    action: "read",
    possession: "any",
  ...
  async findDeadlines(
    ...
    const results = await this.service.findDeadlines(parent.id, args);
  ...
  async getContact(@graphql.Parent() parent: Lead): Promise<Contact | null> {
  ...
  async getCustomer(@graphql.Parent() parent: Lead): Promise<Customer | null> {
